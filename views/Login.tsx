...
import { ChevronRight, Lock, User as UserIcon, Trophy, AlertTriangle } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ users, onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedUserId) {
        setError('Selecciona un guerrero');
        return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 p-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 uppercase text-[10px] font-black tracking-[0.3em]">Legión de Entrenamiento</p>
        </div>

        {users.length === 0 ? (
          <div className="glass rounded-[2rem] p-8 text-center border-orange-500/20">
            <AlertTriangle className="text-orange-500 mx-auto mb-4" size={32} />
            <p className="text-white font-bold mb-2">Sin Guerreros Detectados</p>
            <p className="text-slate-500 text-[10px] uppercase tracking-widest leading-relaxed">
              La base de datos está vacía o las políticas de Supabase (RLS) están bloqueando la conexión.
            </p>
            <button 
              onClick={() => window.location.reload()}
              className="mt-6 text-orange-500 text-[10px] font-black uppercase tracking-widest border-b border-orange-500/20 pb-1"
            >
              Reintentar Conexión
            </button>
          </div>
